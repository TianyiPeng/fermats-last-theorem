...
and the Lean theorem that carries it, and the whole proof is also published as web pages you can browse (see
"Reading the proof in a browser" below).
...
The proof is published as static web pages at <https://tianyipeng.github.io/fermats-last-theorem/>: the route of the
proof step by step; a page for each of the 29,511 theorems (the exact Lean statement, what it cites and what cites it,
and an expandable dependency graph) and for each of the 1,450 definition modules (the full source and which statements
use it); a search box over all theorem and definition names; the landmark theorems as a graph; and `README.md`,
`PROOF-PATH.md` and `ATTRIBUTION.md` rendered with cross-links. They were generated by `tools/docs-site/`, and the
generated folder (about 390 MB, browsable offline from disk with no web server) is held on the repository's `main`
branch rather than here. The pages were machine-tested in a Chromium-based browser only, and the `README-DOCS.md`
beside them explains what is quoted from the Lean files and what is generated (the English summaries and suggested
references are generated automatically; the Lean statement is authoritative).
...
that re-prove a Mathlib lemma in place). The page generator bundles KaTeX and Graphviz (compiled to WebAssembly)
under their own licences, in `tools/docs-site/gen/vendor/`. Lean and the packages in `lake-manifest.json` are fetched
